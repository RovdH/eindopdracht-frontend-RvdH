...
function Home() {
 ...
 const [maxReadyTime, setMaxReadyTime] = useState(30);
    ...
    const fetchRecipe = async () => {
        ...
        try {
            const response = await api.get(`/recipes/complexSearch?${query}&number=1`);
            ...
            <main className={styles.home__main}>
                <form className={styles.home__form_wrapper} onSubmit={(e) => e.preventDefault()}>
                    ...
                    <fieldset className={styles.home__form_btnfields}>
                        <Button variant={"btn_lightgray"} onClick={() => setMaxReadyTime(10)}>No time to waste</Button>
                        <Button variant={"btn_lightgray"} onClick={() => setMaxReadyTime(20)}>Kind of a lot?!?</Button>
                        <Button variant={"btn_lightgray"} onClick={() => setMaxReadyTime(30)}>Meh</Button>
                        <Button variant={"btn_lightgray"} onClick={() => setMaxReadyTime(60)}>Relax</Button>
                    </fieldset>
